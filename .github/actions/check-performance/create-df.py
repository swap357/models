...
if os.path.isfile(f'{OUTPUT_FOLDER}/performances/result-table_{WORKLOAD}_{SCRIPT_NAME}.csv') == False:
    result_df = pd.DataFrame(columns=['Framework', 'Workload', 'Script', 'Precision', 'Benchmark Type', 'Tolerance', 'Threshold', 'Result', 'Pass/Fail'])
    result_df.loc[len(result_df.index)] = [FRAMEWORK, WORKLOAD, SCRIPT, PRECISION, BENCHMARK_TYPE, TOLERANCE, THRESHOLD, RESULT, PASS]
